...
class GPSDataPreprocessor:
    """GPS 資料預處理器"""
    def __init__(self, time_threshold=300, distance_threshold=20.0):
        """初始化預處理器"""
        self.time_threshold = time_threshold
        self.distance_threshold = distance_threshold
    
    def parse_keyname_to_gpstime(self, keyname: str) -> datetime:
        """從 KeyName 解析 GPS 時間"""
        try:
    ...
    def split_into_sequences(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx, row in df.iterrows():
            ...
            if (row['GPSTime_diff'] > self.time_threshold) or (row['distance_to_prev'] > self.distance_threshold):
                group_id += 1
    ...
    def preprocess(self, csv_path: Path) -> pd.DataFrame:
        ...
        df_unique['GPSTime'] = df_unique['KeyName'].apply(self.parse_keyname_to_gpstime)
        ...
        df_unique = self.split_into_sequences(df_unique, self.time_threshold, self.distance_threshold)
        ...
        if (df_unique['GPSTime_diff'] > self.time_threshold).any():
            logger.warning("Warning: Some GPSTime_diff values are greater than 60 seconds.")
            warning_df = df_unique[df_unique['GPSTime_diff'] > self.time_threshold]
