...
if __name__ == '__main__':
    folder = Path(r"E:\10月AIROAD更新\影片")


    files = [f for f in folder.iterdir() if f.suffix == ".MP4"]
    for file in files:
        columns = ["filename", "datetime", "lat", "lon", "speed", "azimuth"]
        df = makeExifDf(file, columns)
        if len(df):
            out = os.path.join(r"E:\10月AIROAD更新\exif", f"{file.stem}.csv")
            saveExifCsv(df, out)
    ...
    # df = makeExifDf(file, columns)
    # if len(df):
    #     out = (r'E:\10月AIROAD更新\exif' / f"{file.stem}.csv")
    #     saveExifCsv(df, out)
